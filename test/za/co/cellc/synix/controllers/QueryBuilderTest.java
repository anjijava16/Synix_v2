...
//        HtmlInputProcessor.getInstance().processInput(new StringBuilder(selectionStr));
    ...
    private List<String> getCellAvailabilityQueries() {
        ...
        queries.add("SELECT BSC_GID,to_char(Period_Start_Time,'yyyy/MM/dd HH24:mi:ss'),100*(decode((NVL(sum(BCCH_UPTIME),0) + NVL(sum(BCCH_DOWNTIME),0)), 0, 0,sum(BCCH_UPTIME)/(NVL(sum(BCCH_UPTIME),0) +NVL(sum(BCCH_DOWNTIME),0)))) FROM N2_CELL_AVAIL_CTRL_D_TEST WHERE Period_Start_Time >= to_date('30/03/2014 00:00:00','dd/mm/yyyy hh24:mi:ss') AND Period_Start_Time <= to_date('29/04/2014 23:00:00','dd/mm/yyyy hh24:mi:ss') AND  Period = 'DAILY' AND  LEVEL_ = 'CONTROLLER' AND (BSC_GID='694806002' OR BSC_GID='676325002' ) GROUP BY BSC_GID,Period_Start_Time ORDER BY BSC_GID,Period_Start_Time");
        return queries;
    }
}
