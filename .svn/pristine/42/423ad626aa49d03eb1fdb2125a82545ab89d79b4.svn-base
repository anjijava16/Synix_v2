var TREE_ID = ["NSN_2G_KPI_Charts", "NSN_3G_KPI_Charts", "NSN_2G_REV_AVAIL", "NSN_3G_REV_AVAIL"];
var TREE_CSS_CLASSNAMES = ["dijitLeaf", "dijitLeaf", "dijitLeaf2", "dijitLeaf2"];
...
// Back-compat shim
    ...
    var Tree = declare("dijit.Tree", [_Widget, _TemplatedMixin], {
        ...
        _setPathsAttr: function(/*Item[][]|String[][]*/ paths) {
            ...
            function setNodes(newNodes) {
                ...
                tree.set("selectedNodes", array.map(
                        array.filter(newNodes, function(x) {
                            return x[0];
                        }),
                        function(x) {
                            return x[1];
        ...
        getIconClass: function(/*dojo/data/Item*/ item, /*Boolean*/ opened) {
            ...
            newClass = (!item || this.model.mayHaveChildren(item)) ? (opened ? "dijitFolderOpened" : "dijitFolderClosed") : "dijitLeaf";
            var len = TREE_ID.length;
            if (newClass === "dijitLeaf") {
                for (i = 0; i < len; i++) {
                    if (item.id === TREE_ID[i]) {
                        newClass = TREE_CSS_CLASSNAMES[i];
                        return newClass;
                    }
                }
            }
